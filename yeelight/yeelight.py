...
def set_power(idx, power, effect, duration, mode):
  param = "\"%s\",\"%s\",%d,%d" % (power,effect,duration,mode)
  operate_on_bulb(idx, "set_power", param)

def set_bright(idx, bright):
  operate_on_bulb(idx, "set_bright", str(bright))

def set_rgb(idx, bright):
  operate_on_bulb(idx, "set_rgb", str(bright))
  
def toggle_msg_handler(action):
  set_power(1, action, "smooth",30, 0)

def bright_msg_handler(action):
  bright = int(float(action))
  print "bright", bright
  set_bright(1, bright)

def color_msg_handler(action):
  color = int(action, 16)
...
controllerBroker.addHandler("yeelight_t", toggle_msg_handler)
controllerBroker.addHandler("yeelight_b", bright_msg_handler)
controllerBroker.addHandler("yeelight_c", color_msg_handler)
